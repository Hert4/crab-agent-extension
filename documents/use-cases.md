...
```
Go to wikipedia.org and find the population of Tokyo
```
...
and body "Hi John, can we reschedule to 3pm? Thanks."
```

```
Open my Gmail inbox and tell me how many unread emails I have
```
...
```
Go to this job application page and fill in my details:
...
```
Read the pricing table on this page and list all plans with their prices
```

```
Go to amazon.com, search for "wireless mouse", and list the top 10 results
...
```
Generate a DOCX report with the title "Weekly Summary" and include
...
```
Open YouTube and search for "learn typescript in 10 minutes"
```
...
sort by price low to high, and show me the first 5 options
```

```
Compare the price of iPhone 16 on amazon.com vs bestbuy.com
```
...
```
Open this pull request and summarize the changes
```
...
```
Search for "React vs Vue 2025 comparison" and summarize the pros and cons
from the top 3 results
```

```
Go to Hacker News, find posts about AI agents, and list the top 5
...
2. Search for round-trip flights from NYC to Tokyo
3. Set dates to next month, first two weeks
4. Sort by cheapest
5. Tell me the top 3 options with prices and airlines
```

```
Open Google Docs, create a new document titled "Meeting Notes",
type today's date as a heading, then add bullet points:
...
```
Watch this page and let me know when the "Apply" button appears
```
