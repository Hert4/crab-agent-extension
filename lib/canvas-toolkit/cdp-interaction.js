 ...
 * Simulates hardware via Chrome DevTools Protocol
 */
...
class CDPInteraction {
  ...
  /**
   * CDP Click - Move mouse and click at coordinates [x, y]
   */
  ...
  /**
   * CDP Drag - Drag from point A to point B
   */
  async cdp_drag(startX, startY, endX, endY, options = {}) {
    ...
    try {
      ...
      // 3. Gradually move to end point (simulates real drag)
  ...
  /**
   * CDP Type - Type each character one by one
   */
  ...
  /**
   * CDP Press Key - Press special keys (Enter, Tab, Escape, etc.)
   */
